...
実際どのような場面で使う必要があるのか考えましょう。<br/>Starbucksのサイトを例にします。

<img src="./img/starbucks-lp.png" width="100%">

左からサイト全体、コンポーネント、インスタンスです。<br/>サイト全体を見ると、同じようなレイアウトのデザインパーツが並んでいる部分があります。<br/>例えば、「メニューの飲み物や食べ物のカード」や「カテゴリーの項目」です。<br/>この同じようなデザインパーツの基本構造を作成したものが`コンポーネント`です。コンポーネントを1つの型として作成すると、あとは画像やテキスト内容を変えて複製するだけで他の同じようなパーツを作成することができます。この複製したものを`インスタンスと言います。

Figma のコンポーネント・インスタンス機能では、作成したパーツを`コンポーネント化`し、それを複製して`インスタンス`を生成することができます。
コンポーネント(親)を複製して作られたインスタンス(子)は、コンポーネント(親)と紐づけられ、コンポーネントの修正がインスタンスにも反映されます。
デザイン変更でインスタンス1つ1つを修正せずに、コンポーネントの修正だけで済むケースがあるので、作業の効率化とアプリ全体の統一性を保つことができます。  
